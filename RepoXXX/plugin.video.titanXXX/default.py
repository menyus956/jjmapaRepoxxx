...
import threading
hosts = ["https://archive.org/download/NaviXPlaylist/TITANX.xml"]
try: action = dict(urlparse.parse_qsl(sys.argv[2].replace('?','')))['action']
except: action = None
addon_id = 'plugin.video.titanXXX'
selfAddon = xbmcaddon.Addon(id=addon_id)
addon = Addon(addon_id, sys.argv)
fanart = xbmc.translatePath(os.path.join('special://home/addons/' + addon_id , 'fanart.jpg'))
icon = xbmc.translatePath(os.path.join('special://home/addons/' + addon_id , 'art/kiss.png'))
fanartmm = xbmc.translatePath(os.path.join('special://home/addons/' + addon_id , 'art/myass.jpg'))
...
def CATEGORIES():
	Check_haters()		
	addDir2('[COLOR blue]T[/COLOR]ITAN [COLOR red]XXX[/COLOR]','https://archive.org/download/NaviXPlaylist/TITANX.xml',6,icon,fanartmm,fanart)
...
def Check_haters():
		safe_to_go = 0
		name = "plugin.video.titanXXX"
		import shutil
		for root, dirs, files in os.walk(HOME,topdown=True):
				if "spartan" in str(dirs):
					dialog.ok('[COLOR lime][B]Titan[/B][/COLOR][COLOR white] Autodetect[/COLOR] ','TitanXXX addon doesnt support scammers and dishonest reviewers... If you have installed anything related to spartanpixel this addon will not work for you, and you should use a different build, plenty of free builds out there','','')
					for dir in dirs:
